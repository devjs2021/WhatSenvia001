...
export function DashboardTopbar({ onMobileMenuToggle }: DashboardTopbarProps) {
  ...
  return (
    <header className="border-b border-slate-100 bg-white shrink-0">
      ...
      {/* Sub-navegación del grupo activo (dentro del header) */}
      {activeGroup && activeGroup.children.length > 1 && (
        <nav className="flex items-center gap-1 px-6 md:px-10 pb-3 overflow-x-auto">
          {activeGroup.children.map((child) => {
            ...
            return (
              ...
              >
                <child.icon className="w-4 h-4 shrink-0 text-emerald-500" strokeWidth={1.5} />
